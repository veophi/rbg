...
func RunRbgScalingAdapterControllerTestCases(f *framework.Framework) {
	ginkgo.Describe("rbg controller", func() {
		...
		ginkgo.It("test role with scalingAdapter", func() {
			rbg := wrappers.BuildBasicRoleBasedGroup("e2e-test", f.Namespace).
				WithRoles(
					[]workloadsv1alpha1.RoleSpec{
						wrappers.BuildBasicRole("role-1").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).WithScalingAdapter(true).Obj(),
						wrappers.BuildBasicRole("role-2").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).WithScalingAdapter(false).Obj(),
						wrappers.BuildBasicRole("role-3").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).Obj(),
					}).Obj()
		...
		ginkgo.It("test role with scalingAdapter and update rbg to delete the role", func() {
			rbg := wrappers.BuildBasicRoleBasedGroup("e2e-test", f.Namespace).
				WithRoles(
					[]workloadsv1alpha1.RoleSpec{
						wrappers.BuildBasicRole("role-1").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).WithScalingAdapter(true).Obj(),
						wrappers.BuildBasicRole("role-2").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).WithScalingAdapter(true).Obj(),
						wrappers.BuildBasicRole("role-3").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).Obj(),
					}).Obj()
			gomega.Expect(f.Client.Create(f.Ctx, rbg)).Should(gomega.Succeed())
		...
		ginkgo.It("test update rbg to add a new role with scalingAdapter enabling", func() {
			rbg := wrappers.BuildBasicRoleBasedGroup("e2e-test", f.Namespace).
				WithRoles(
					[]workloadsv1alpha1.RoleSpec{
			...
			newRbg.Spec.Roles = append(newRbg.Spec.Roles, wrappers.BuildBasicRole("role-2").
		...
		ginkgo.It("test update role.scalingAdapter.enable from true to false and nil", func() {
			rbg := wrappers.BuildBasicRoleBasedGroup("e2e-test", f.Namespace).
				WithRoles(
					[]workloadsv1alpha1.RoleSpec{
						wrappers.BuildBasicRole("role-1").WithWorkload(workloadsv1alpha1.StatefulSetWorkloadType).WithScalingAdapter(true).Obj(),
		...
		ginkgo.It("test scale role in rbg", func() {
			rbg := wrappers.BuildBasicRoleBasedGroup("e2e-test", f.Namespace).
				WithRoles(
					[]workloadsv1alpha1.RoleSpec{
			...
			gomega.Expect(f.Client.Get(f.Ctx, client.ObjectKey{
				...
				Namespace: rbg.Namespace,
			}, rbgSa)).Should(gomega.Succeed())

			scale := &autoscalingv1.Scale{}
			gomega.Expect(f.Client.SubResource("scale").Get(f.Ctx, rbgSa, scale)).Should(gomega.Succeed())
			newReplicas := int32(2)
			scale.Spec.Replicas = newReplicas
			gomega.Expect(f.Client.SubResource("scale").Update(f.Ctx, rbgSa, client.WithSubResourceBody(scale))).Should(gomega.Succeed())
